...
class LoadingDone(Exception):
    " Excepcion para referirse el fin del cargo de bits"
...
def encode(message, in_img, out_img="_stegaed.png"):
    print("Codificando ({}): {}".format(len(message), message))

    if len(message) > 255:
        print("Mensaje muy grande.")
        return -1
    ...
    try:
        ...
        print("Guardar: ", out_img)

    image.save(out_img)

def decode(img):
    print("Decodificando:", img)
